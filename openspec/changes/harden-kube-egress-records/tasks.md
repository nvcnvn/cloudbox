...
- [ ] 3.1 Add the proxy's startup incarnation identifier to the `/attempts` response and carry it through `EgressAttempts`; give the proxy deployment the resource requests, limits and readiness probe that keep a restart an event rather than routine
